...
export class CrawlerStatistics {
    ...
    /**
     * M8 FIX (2026-08-18): idempotent ingest for content-script telemetry
     * batches. The observer flushes delta snapshots with a per-batch id and
     * retransmits the SAME batch verbatim until it receives an ack; when the
     * SW recorded a batch but the ack was lost, the retransmission must not
     * be counted twice — we dedup on batchId here.
     *
     * The seen-set is IN MEMORY BY DESIGN: it shares the lifetime of the
     * counters it protects (this instance — M8-BUG6 removed Statistics
     * persistence as an anti-pattern). If the SW is evicted, counters and
     * seen-ids are wiped together, so re-ingesting a retransmitted batch into
     * the fresh instance is RECOVERY of lost data, not double counting.
     * Persisting the ids without the counters would turn that recovery into
     * permanent loss.
     *
     * Legacy compat: payloads without a batchId are recorded unconditionally
     * (pre-M8 sender behavior).
     *
     * @param {{batchId?: string, snapshot: Array}} payload
     * @returns {{ok: boolean, deduped: boolean}}
     */
    ingestSelectorTelemetryBatch(payload) {
        const snapshot = payload && payload.snapshot;
        if (!Array.isArray(snapshot) || snapshot.length === 0) return { ok: true, deduped: false };
        const batchId = (payload && typeof payload.batchId === 'string' && payload.batchId.length > 0)
            ? payload.batchId : null;
        if (!this._seenTelemetryBatchIds) this._seenTelemetryBatchIds = new Set();
        if (batchId && this._seenTelemetryBatchIds.has(batchId)) {
            // Replay of an already-recorded batch (ack was lost): ack again,
            // record nothing.
            return { ok: true, deduped: true };
        }
        this.recordSelectorTelemetry(snapshot, { deltaMode: false });
        if (batchId) {
            this._seenTelemetryBatchIds.add(batchId);
            // Bounded memory: FIFO prune. At the 60s flush cadence, 500 ids
            // cover ~8h of batches per sender — far beyond any plausible
            // retransmission window.
            if (this._seenTelemetryBatchIds.size > 500) {
                const oldest = this._seenTelemetryBatchIds.values().next().value;
                this._seenTelemetryBatchIds.delete(oldest);
            }
        }
