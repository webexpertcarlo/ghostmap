...
// (Still used by the JSON and Markdown export paths; the CSV path now
// delegates wholesale — see the data-exporter import below.)
import { cleanEmailsForCsv, deriveScrapeStatus, escapeMarkdownCell } from './exportSanitize.js';
// S12 (2026-08-18): the CSV artifact has ONE generator. exportAsCSV used to
// hand-roll a divergent 13-column CSV (no out-of-radius discard, no BOM,
// 'Status' instead of 'Scrape Status') over the same DB the UI exports with 57
// columns. It now delegates to background/data-exporter.js generateCSV +
// selectExportRows — same columns, same radius filter, same escaping/formatters.
// Layering note: lib → background import is deliberate and cycle-free
// (data-exporter only imports lib/ modules); moving the generator into lib/
// would be a larger relocation than this fix warrants.
import { generateCSV, selectExportRows } from '../background/data-exporter.js';
 ...
 *
 * S12 (2026-08-18) — BREAKING CHANGE for api_export_csv consumers: the CSV is
 * now generated by the SAME function as the UI export (background/
 * data-exporter.js generateCSV). Effects:
 *   • 13 → 57 columns (full schema; 'Status' is now 'Scrape Status', column
 *     order follows the UI export — the old 13 were NOT a prefix).
 *   • Provably out-of-radius rows are discarded (selectExportRows), exactly
 *     like the UI export. `count` is the POST-filter row count; the additive
 *     `discarded` field reports the drop count (BUG-6 honesty semantics).
 *   • The payload is prefixed with a UTF-8 BOM — the same byte the UI download
 *     prepends (ui/sidepanel.js downloadFile, EXP-03) — so writing `data` to a
 *     .csv file yields an Excel-ready artifact identical to the UI's.
 * Compat assessment: no externally_connectable/onMessageExternal surface
 * exists; api_export_csv is reachable only from extension pages/console with
 * the API key and the 2026-05-07 dead-code census found no consumers.
 * Query filters/sort (an API-only capability) still apply BEFORE generation.
 *
...
export async function exportAsCSV(params = {}) {
    ...
    try {
        ...
        // S12: single CSV truth — the UI generator builds the artifact
        // (57 columns, out-of-radius discard, BUG-5 email/status sanitization,
        // F8-b/BR-1 escaping — all enforced INSIDE generateCSV). The BOM is the
        // same one the UI download path prepends (downloadFile guards against
        // double-prepending, so this byte is safe on every consumer path).
        const csv = '﻿' + generateCSV(businesses);
        // BUG-6 semantics: count = rows actually written (same partition
        // generateCSV uses), discarded = out-of-radius drops.
...
export async function exportAsMarkdown(params = {}) {
    ...
    try {
        ...
        if (emailsOnly) {
            ...
            businesses.forEach(b => {
                const email = escapeMarkdownCell(cleanEmailsForCsv(b.email) || '-');
                const title = escapeMarkdownCell(b.title || 'Unknown');
                const category = escapeMarkdownCell(b.category || '-');
                const website = escapeMarkdownCell(b.website || '-');
                const phone = escapeMarkdownCell(b.phone || '-');
...
export async function exportAsJSON(params = {}) {
    ...
    try {
        ...
        // Clean emails + derive an explicit scrape status.
        // E4 (ATP 2026-07-17): emailScraped:true with an empty email was
        //   ambiguous ("scraped, none found" vs "not yet scraped"); the derived
        //   scrapeStatus makes each JSON record self-describing.
        // X3 (ATP 2026-07-17): the JSON export is the FULL-FIDELITY record BY
        //   DESIGN — it spreads {...b} and deliberately keeps DB-side lead
        //   attributes (reviewCount, hoursRaw) that the curated CSV omits.
        //   Reconciled per the tracker note ("outside the CSV UI; present in
        //   JSON"): those fields are useful for programmatic consumers, and
        //   scrapeStatus above makes the extra shape intentional, not a leak.
        const cleanedData = queryResult.data.map(b => {
            const cleanedEmail = cleanEmailsForCsv(b.email);
            return {
                ...b,
                email: cleanedEmail,
                scrapeStatus: deriveScrapeStatus(b, cleanedEmail),
            };
        });
